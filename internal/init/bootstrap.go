...
func printCompletion(w io.Writer, cfg *config.Config, opts Options) {
	...
	fmt.Fprintln(w, "  1. Set any required secrets:")
	fmt.Fprintln(w, `     herald secret set <stack>/<key> "value"`)
	fmt.Fprintln(w)
	fmt.Fprintln(w, "  2. Start the herald daemon (wires Caddy + webhooks on first start):")
	fmt.Fprintln(w, "     sudo systemctl enable --now herald")
	fmt.Fprintln(w)
